...
module Semver
  ...
  def self.sort(list)
    loop do
      ...
      (1..list.size - 1).each do |i|
        next unless Semver::Comparison.new(list[i - 1], list[i]).gt
        tmp = list[i].dup
        list[i] = list[i - 1].dup
        list[i - 1] = tmp
        flag = true
  ...
  def self.rsort(list)
    loop do
      flag = false
      (1..list.size - 1).each do |i|
        next unless Semver::Comparison.new(list[i - 1], list[i]).lt
        tmp = list[i]
        list[i] = list[i - 1]
        list[i - 1] = tmp
        flag = true
  ...
  def self.valid_range(range)
    Semver::Range.new(range).valid_range
  end
  ...
  def self.max_satisfying(versions, range)
    satisfied_versions = []
    versions.each do |v|
      satisfied_versions << v if Semver.satisfies(v, range)
    end
